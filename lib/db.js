...
function migrate() {
  ...
  addColumn('settings', 'questions_per_assessment', 'INTEGER');
}
...
export function getSettings() {
  ...
  return {
    ...
    questionsPerAssessment: row.questions_per_assessment,
...
export function updateSettings({ title, quarter, secondsPerQuestion, showScoreToOfficer, showReviewToOfficer, questionsPerAssessment }) {
  getDb()
    .prepare(
      `UPDATE settings SET title = ?, quarter = ?, seconds_per_question = ?, show_score_to_officer = ?,
        show_review_to_officer = ?, questions_per_assessment = ? WHERE id = 1`
    )
    .run(title, quarter, secondsPerQuestion, showScoreToOfficer ? 1 : 0, showReviewToOfficer ? 1 : 0, questionsPerAssessment || null);
}
